        ...
        <div class="col-4 px-0 bg-white">
            <div class="card border-0">
                        ...
                        <div class="col-auto">
                            <div class="dropdown">
                                ...
                                @if (Auth::user()->id === $post->user->id)
                                                     {{-- $post comes from Homecontroller --}}
                                    <div class="dropdown-menu">
                                        ...
                                        <i class="fa-regular fa-trush-can"></i> Delete
                                        </button>
                                    </div>
                                    {{-- Include modal here --}}
                                    @include('users.posts.contents.modal.delete')
                                @else
                                    {{-- if you are NOT the OWNER of the post, show an UNFOLLOW("FOLLOWING") button. [Soon] --}}
                                    <div class="dropdown-menu">
                                        @if ($post->user->isFollowed())
                                        <form action="{{ route('follow.destroy', $post->user->id ) }}" method="post">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="dropdown-item text-danger border-0 bg-transparent text-secondary p-1">Following</button>
                                        </form>
                                    @else
                ...
                <div class="card-body w-100">
                    {{-- heart button + no. of links and categories --}}
                    <div class="row align-items-center">
                        <div class="col-auto">
                            @if ($post->isLiked())
                        ...
                        <div class="col text-end">
                            @forelse ($post->categoryPost as $category_post)
                                        {{-- categoryPost method from Post.php --}}
